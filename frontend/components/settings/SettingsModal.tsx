...
export default function SettingsModal(props: SettingsModalProps) {
  ...
  const handleClose = () => {
    ...
    setTimeout(() => {
      setSettingsOpen(false); // Actually close it after the animation finishes
    }, 200);
  };
  ...
  return (
      <div className={`fixed inset-0 z-[60] flex items-center justify-center bg-black/60 px-4 ${isClosing ? "animate-fade-out" : "animate-fade-in"}`}>
        <div
            className={`${isClosing ? "animate-scale-pop-out" : "animate-scale-pop"} ${t.modalBg} border ${t.border} rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden max-h-[90vh] md:max-h-none`}
        ...
        >
          ...
          <div className={`${t.modalSidebar} border-b md:border-b-0 md:border-r ${t.border} flex flex-row md:flex-col overflow-x-auto md:w-44 shrink-0`}>
            ...
            <button onClick={handleClose} className={`hidden md:block mx-4 mt-4 text-xs ${t.textMuted} hover:text-red-400 transition-colors text-left pb-4`}>✕ Close</button>
          </div>
