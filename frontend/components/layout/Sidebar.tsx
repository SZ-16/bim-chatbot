...
export default function Sidebar(props: SidebarProps) {
  ...
  return (
      <div className={`
      ${SIDEBAR_WIDTH_MAP[sidebarWidth]} ${t.sidebarBg} border-r ${t.border} 
      flex flex-col shrink-0 transition-all z-40
      ...
      `}>
        ...
        <div className="flex-1 overflow-y-auto px-2">
          <p className={`${t.textMuted} px-2 mb-2`} style={{fontSize: `${Math.max(fontSize - 2, 10)}px`}}>Recent
            Chats</p>

          {chats.length === 0 && (
              <p className={`${t.textMuted} px-2`} style={{fontSize: `${Math.max(fontSize - 2, 10)}px`}}>No chats
                yet.</p>
          )}
